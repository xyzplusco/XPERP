...
import { createTaskAction, getTaskOptionsAction } from "@/lib/actions";
...
// 담당자·프로젝트 목록은 과제 창을 처음 열 때만 가져온다.
...
export function TaskDialog() {
  ...
  useEffect(() => {
    ...
    if (loaded) return;
    let alive = true;
    void getTaskOptionsAction().then((data) => {
      if (!alive) return;
      ...
      {open ? (
        <div className="modalBackdrop" onClick={() => setOpen(false)}>
          <div className="modalCard" onClick={(event) => event.stopPropagation()}>
            ...
            <form action={createTaskAction} className="modalBody">
