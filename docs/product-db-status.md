...
`npm run db:audit` 기준, reconciliation 적용 후:
...
| 프로젝트 → PL | 63 / 195 |
| 프로젝트 → PM | 34 / 195 |
| 액션 → 고객사/프로젝트/사람/이벤트/문서 | 98 / 151 |
| 문서 필요 → 사람/회사/프로젝트/이벤트/태스크 | 239 / 245 |
| 고객사 enrich 필드 | 105 |
...
2026-08-17 승인 후 reconciliation 적용 결과:
...
| 이슈 | 처리 |
| --- | --- |
| `회사명` 헤더 row | 원천 DB에는 보존, 제품 view에서는 제외 |
| `A사`, `B사` 등 익명 M&A 딜 | 삭제하지 않고 익명 딜로 남김. 이후 별도 deal codename 정책 필요 |
| 실제 문서 파일 0건 | Supabase Storage 및 업로드 플로우 필요 |
